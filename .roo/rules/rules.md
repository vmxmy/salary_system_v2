...
### 任务概述：
开发完整的一键导出报表功能，包括数据库存储优化、API完善和前端功能实现。
...
### 技术要点：
- 数据库设计：报表模板、配置、生成记录
- API设计：RESTful接口、异步处理、文件管理
- 前端实现：React组件、状态管理、文件下载
...
### 阶段1分析结果：
**现有报表存储结构**：
- ✅ **基础表结构完善**：已有 `config.report_data_sources`、`config.report_templates`、`config.report_executions` 等核心表
- ✅ **报表视图支持**：已有 `reports.report_views` 和 `reports.report_view_executions` 表
- ✅ **权限控制**：已有 `config.report_permissions` 和用户偏好设置
- ⚠️ **批量导出优化**：缺少批量报表生成任务管理和进度跟踪
- ⚠️ **文件管理**：缺少统一的文件存储和清理机制
