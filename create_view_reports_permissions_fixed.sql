-- 视图报表模块权限创建脚本（修正版）
-- 创建时间: 2024年
-- 说明: 根据实际系统角色为视图报表模块创建必要的权限和角色分配
...
SELECT 
    ...
    name,
    CASE 
        WHEN code = 'SUPER_ADMIN' THEN '拥有所有权限'
        WHEN code = 'report_admin' THEN '报表管理员 - 完整管理权限'
        WHEN code = 'HR' THEN '人事负责人 - 创建编辑权限'
        WHEN code = 'FA' THEN '财务 - 创建编辑权限'
        WHEN code = 'report_user' THEN '报表用户 - 只读权限'
        ELSE '未分配视图报表权限'
...
-- 完成提示
SELECT '视图报表模块权限创建完成！已根据实际角色进行分配。' as status; 