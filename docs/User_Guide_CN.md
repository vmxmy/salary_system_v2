...
**含义**:
工资发放运行（也常被称为薪资审核或薪资核算任务）是指针对一个特定的工资期间，为特定范围的员工执行实际薪资计算和生成支付指令的过程。一次"运行"会基于该工资期间的员工数据、考勤、绩效、各项薪资政策（如工资组成项、税率、社保公积金规则等）来计算每个员工的应发合计、各项扣除以及实发合计。
...
*   **查看运行详情 (View Details)**: 
    *   这是非常重要的一步，点击后会进入该工资发放运行的详细页面（即"薪资审核详情"）。
...
#### 4.2.3 薪资审核详情 (Payroll Run Details)
