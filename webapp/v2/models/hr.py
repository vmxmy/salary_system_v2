...
class Employee(BaseV2):
    ...
    department_id = Column(BigInteger, ForeignKey('hr.departments.id', ondelete='SET NULL'), nullable=True)
    job_title_id = Column(BigInteger, ForeignKey('hr.job_titles.id', ondelete='SET NULL'), nullable=True)
    ...
    current_department = relationship("Department", foreign_keys=[department_id])
    current_job_title = relationship("JobTitle", foreign_keys=[job_title_id])
