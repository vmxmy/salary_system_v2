...
class ReportDataSourceCreate(ReportDataSourceBase):
    # fields: Optional[List[ReportDataSourceFieldCreate]] = Field(default_factory=list, description="字段列表")  # 已移除字段表
    pass
...
class ReportDataSource(ReportDataSourceBase):
    id: int
    field_count: Optional[int] = Field(0, description="字段数量")
    ...
    enable_dynamic_fields: bool = Field(True, description="是否启用动态字段")
    field_grouping_enabled: bool = Field(True, description="是否启用字段分组")
    auto_infer_categories: bool = Field(True, description="是否自动推断分类")
    
    # 关联数据 - 已移除字段表
    # fields: List[ReportDataSourceField] = Field([], description="字段列表")  # 已移除字段表
...
class ReportTypeDefinitionBase(BaseModel):
    ...
    # 数据源和字段配置
    data_source_id: Optional[int] = Field(None, description="关联的数据源ID")
    fields: Optional[str] = Field(None, description="字段列表(逗号分隔的字段ID)")
...
class ReportDataPreviewResponse(BaseModel):
    total: int
