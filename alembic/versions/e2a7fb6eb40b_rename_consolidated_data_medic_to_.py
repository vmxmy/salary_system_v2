"""rename_consolidated_data_medic_to_medical

Revision ID: e2a7fb6eb40b
Revises: 3b780ced3668
Create Date: 2025-05-07 17:23:22.401027
...
revision: str = 'e2a7fb6eb40b'
down_revision: Union[str, None] = '3b780ced3668'
...
def upgrade() -> None:
    """Upgrade schema."""
    # 重命名 consolidated_data 表中的字段
    op.alter_column('consolidated_data', 'med_medic_total_employer_contribution',
                    new_column_name='med_medical_total_employer_contribution',
                    schema='staging')
    op.alter_column('consolidated_data', 'med_medic_total_employee_contribution',
                    new_column_name='med_medical_total_employee_contribution',
                    schema='staging')

    # 更新 Python 模型中的字段名称
    # 注意：这里不需要额外的代码，因为我们已经在前面的迁移中更新了模型
    ...
    SELECT
        ...
        -- 社保信息
        cd.med_medical_total_employee_contribution AS "医疗保险个人应缴总额",
        ...
        (
            COALESCE(cd.med_medical_total_employee_contribution, 0) +
...
def downgrade() -> None:
    """Downgrade schema."""
    # 恢复原来的字段名
    op.alter_column('consolidated_data', 'med_medical_total_employer_contribution',
                    new_column_name='med_medic_total_employer_contribution',
                    schema='staging')
    op.alter_column('consolidated_data', 'med_medical_total_employee_contribution',
                    new_column_name='med_medic_total_employee_contribution',
                    schema='staging')
    ...
    SELECT
        ...
        -- 社保信息
        cd.med_medic_total_employee_contribution AS "医疗保险个人应缴总额",
        cd.pen_pension_employee_contribution AS "养老保险个人应缴金额",
        cd.ann_annuity_employee_contribution AS "职业年金个人应缴费额",
        cd.hf_housingfund_employee_contribution AS "住房公积金个人应缴费额",
        cd.sal_other_deductions AS "其他扣款",
        ...
        (
            COALESCE(cd.med_medic_total_employee_contribution, 0) +
            COALESCE(cd.pen_pension_employee_contribution, 0) +
            COALESCE(cd.ann_annuity_employee_contribution, 0) +
            COALESCE(cd.hf_housingfund_employee_contribution, 0) +
            COALESCE(cd.sal_other_deductions, 0)
