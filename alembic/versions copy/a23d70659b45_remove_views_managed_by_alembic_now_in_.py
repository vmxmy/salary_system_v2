"""Remove views managed by alembic now in dbt

Revision ID: a23d70659b45
Revises: 21edb53a3f59
Create Date: 2024-08-06 11:00:00.000000 # Placeholder date
...
revision: str = 'a23d70659b45'
down_revision: Union[str, None] = '21edb53a3f59' # Depends on the previous state where Alembic created the views
branch_labels: Union[str, Sequence[str], None] = None
depends_on: Union[str, Sequence[str], None] = None

# SQL Definitions copied from migration 21edb53a3f59 for the downgrade function
# These represent the state *before* this migration's upgrade runs.

view_base_data_downgrade_sql = """
...
view_level1_calculations_downgrade_sql = """
CREATE OR REPLACE VIEW view_level1_calculations AS
WITH base_data AS (
    SELECT * FROM view_base_data -- Uses the view recreated above
),
calculations AS (
    SELECT
        *,
...
def upgrade() -> None:
    # Drop views previously managed by Alembic, now managed by dbt
    op.execute("DROP VIEW IF EXISTS public.view_level1_calculations;")
    op.execute("DROP VIEW IF EXISTS public.view_base_data;")


def downgrade() -> None:
    # Restore views previously managed by Alembic
    # Create view_base_data first
    op.execute(view_base_data_downgrade_sql)
    # Then create view_level1_calculations which depends on view_base_data
    op.execute(view_level1_calculations_downgrade_sql)
