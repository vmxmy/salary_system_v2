...
# THIS IS A MODIFIED VERSION FOR "CLEAN START" DIAGNOSTICS:
# - Uvicorn runs WITHOUT --reload
# - Uvicorn runs WITH --workers 1

echo "Starting frontend and backend development servers (CLEAN MODE)..."

# 创建一个临时脚本来使用zsh激活conda环境并运行后端
if [ -f ".env" ]; then
  source .env
  if [ -n "$CONDA_ENV" ]; then
    echo "Creating temporary script to activate Conda environment with zsh: $CONDA_ENV (CLEAN MODE)"

    # 创建临时脚本 (修改Uvicorn命令)
    cat > temp_run_backend_clean.sh << 'EOL'
#!/usr/bin/env zsh
# 获取conda环境名称
CONDA_ENV=$1
HOST=$2
...
EOL
    ...
    # 使脚本可执行
    chmod +x temp_run_backend_clean.sh

    # 设置后端命令为使用zsh运行临时脚本
    export USE_ZSH_CONDA_CLEAN=true
  fi
fi
...
if ! npm list -g concurrently --depth=0 | grep concurrently > /dev/null; then
  ...
  if ! npm list -g concurrently --depth=0 | grep concurrently > /dev/null; then
    ...
    echo "  Terminal 2 (Navigate to salary_system): uvicorn webapp.main:app --host $UVICORN_HOST --port $UVICORN_PORT --workers 1 --log-level debug"
    echo "--------------------------------------------------------------------------"
    exit 1
  fi
...
FRONTEND_CMD="cd frontend/salary-viewer && npm run dev > ../../frontend-clean.log 2>&1"

# 根据是否使用zsh来选择后端启动命令
if [ "$USE_ZSH_CONDA_CLEAN" = true ]; then
  # 使用临时脚本启动后端 (CLEAN MODE)
  BACKEND_CMD="./temp_run_backend_clean.sh $CONDA_ENV $UVICORN_HOST $UVICORN_PORT"
  echo "Starting Backend with zsh and conda env $CONDA_ENV on: $UVICORN_HOST:$UVICORN_PORT (CLEAN MODE: no --reload, --workers 1, debug log to backend-clean.log)"
else
  # 使用默认命令启动后端 (CLEAN MODE: no --reload, add --workers 1)
  BACKEND_CMD="uvicorn webapp.main:app --host $UVICORN_HOST --port $UVICORN_PORT --workers 1 --log-level debug > backend-clean.log 2>&1"
  echo "Starting Backend on: $UVICORN_HOST:$UVICORN_PORT (CLEAN MODE: no --reload, --workers 1, debug log to backend-clean.log)"
fi

echo "Starting Frontend (output to frontend-clean.log)..."

# Run concurrently
# --kill-others attempts to kill other processes if one exits
# --names adds prefixes to the log output (e.g., "[FRONTEND]", "[BACKEND]")
concurrently --kill-others --names "FRONTEND_CLEAN,BACKEND_CLEAN" "$FRONTEND_CMD" "$BACKEND_CMD" &
CONCURRENTLY_PID=$!

# 等待一段时间，让服务器启动
echo "Waiting for servers to start (CLEAN MODE)..."
sleep 5

# 开始 tail 后端日志
echo "Starting to tail backend logs (CLEAN MODE, from backend-clean.log) (Press Ctrl+C to stop)..."
if [ "$USE_ZSH_CONDA_CLEAN" = true ]; then
...
# Cleanup temporary script if it was created
if [ "$USE_ZSH_CONDA_CLEAN" = true ] && [ -f "temp_run_backend_clean.sh" ]; then
    echo "Removing temporary backend script: temp_run_backend_clean.sh"
    rm temp_run_backend_clean.sh
fi


if [ $EXIT_CODE -ne 0 ]; then
  echo "One or both of the servers failed to start or exited unexpectedly (CLEAN MODE) (Exit Code: $EXIT_CODE)."
else
