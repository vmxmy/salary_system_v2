...
const PayrollEntryDetailModal: React.FC<PayrollEntryDetailModalProps> = ({ entryId, visible, onClose }) => {
  ...
  useEffect(() => {
    if (visible) {
      console.log({t('payroll:auto_____f09f9a')});
      setComponentDefinitionsLoaded(false);
      
      // 检查是否已经有组件定义
      if (payrollConfig.componentDefinitions.length > 0) {
        console.log({t('payroll:auto____f09f92')}, payrollConfig.componentDefinitions.length, {t('payroll:auto_text_e4b8aa')});
      ...
      const loadComponentDefinitions = async () => {
        try {
          console.log({t('payroll:auto___fetchcomponentdefinitions__f09f8c')});
          await payrollConfig.fetchComponentDefinitions();
          console.log({t('payroll:auto____e29c85')}, payrollConfig.componentDefinitions.length, {t('payroll:auto_text_e4b8aa')});
          console.log({t('payroll:auto____f09f92')}, payrollConfig.componentDefinitions.map(def => ({ code: def.code, name: def.name, type: def.type })));
          setComponentDefinitionsLoaded(true);
        } catch (err) {
  ...
  const getComponentDisplayName = (code: string): string => {
    ...
    if (definition) {
      console.log({t('payroll:auto___code_definition_name__f09f94')});
      return definition.name;
    }
    console.warn({t('payroll:auto___code__payrollconfig_componentdefinitions_length__e29aa0')});
    console.warn({t('payroll:auto____e29aa0')}, payrollConfig.componentDefinitions.map(def => def.code));
  ...
  const fetchEmployeeInfo = async (employeeId: number) => {
    ...
    console.log({t('payroll:auto___id__f09f94')}, employeeId);
    ...
    try {
      ...
      if (employee) {
        ...
        console.log({t('payroll:auto____e29c85')}, info);
      } else {
        console.warn({t('payroll:auto___id__e29aa0')}, employeeId);
  ...
  const fetchEntryDetails = async (id: string) => {
    ...
    try {
      ...
      // 检查原始API响应中的所有顶级字段
      console.log({t('payroll:auto__api__f09f94')}, Object.keys(response.data));
