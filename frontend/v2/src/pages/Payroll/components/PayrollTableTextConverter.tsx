...
const PayrollTableTextConverter: React.FC = () => {
  ...
  return (
    <Card title={t('batch_import.table_converter.title', {t('payroll:auto_text_e8a1a8')})}>
      <div style={{ marginBottom: 16 }}>
        <TextArea
          rows={10}
          value={tableText}
          onChange={e => setTableText(e.target.value)}
          placeholder={t('batch_import.table_converter.placeholder', {t('payroll:auto____id_______e8afb7')})}
        />
      </div>
      
      <Button type="primary" onClick={parseTableText}>
        {t('batch_import.table_converter.parse_table', {t('payroll:auto_text_e8a7a3')})}
      </Button>
      ...
      {parsedData.length > 0 && (
        <>
          <Card title={t('batch_import.table_converter.field_mapping', {t('payroll:auto_text_e5ad97')})} style={{ marginTop: 16 }}>
            <EnhancedProTable
              dataSource={fieldMappings.map((m, i) => ({ ...m, key: i }))}
              columns={[
                {
                  title: t('batch_import.table_converter.table_field', {t('payroll:auto_text_e8a1a8')}),
                  dataIndex: 'tableField',
                  valueType: 'text',
                },
                {
                  title: t('batch_import.table_converter.api_field', {t('payroll:auto_api_415049')}),
                  dataIndex: 'apiField',
                  valueType: 'select',
                  render: (_, record: any) => (
                    <Select
                    ...
                    >
                      <Option value="">{t('batch_import.table_converter.ignore_field', {t('payroll:auto_text_e5bfbd')})}</Option>
                      {defaultApiFields.map(field => (
                        <Option key={field.key} value={field.key}>
                          {field.label} {field.required ? `(${t('common:required', {t('payroll:auto_text_e5bf85')})})` : ''}
                        </Option>
                      ))}
                    </Select>
                  )
                ...
                {
                  title: t('batch_import.table_converter.data_type', {t('payroll:auto_text_e695b0')}),
                  dataIndex: 'type',
                  valueType: 'text',
          ...
          <Button
            ...
            style={{ marginTop: 16 }}
          >
            {t('batch_import.table_converter.convert_to_json', {t('payroll:auto_json_e8bdac')})}
          </Button>
          
          {jsonResult && (
            <Card title={t('batch_import.table_converter.json_result', {t('payroll:auto_json_4a534f')})} style={{ marginTop: 16 }}>
              <TextArea
                rows={10}
                value={jsonResult}
                readOnly
              ...
              <Space style={{ marginTop: 8 }}>
                ...
                >
                  {t('batch_import.table_converter.copy_json', {t('payroll:auto_json_e5a48d')})}
                </Button>
                <Button
                  type="primary"
                  onClick={() => {
                    try {
                      ...
                      setError(t('batch_import.table_converter.json_parse_error', {t('payroll:auto_json__4a534f')}));
                    }
                  }}
                >
                  {t('batch_import.table_converter.use_json', {t('payroll:auto_json_e4bdbf')})}
                </Button>
              </Space>
            </Card>
          )}
